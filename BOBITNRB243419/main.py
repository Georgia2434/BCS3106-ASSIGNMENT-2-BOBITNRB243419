...
def def_full_file_reader():  # mustafa
    ...
    while i <= (len(
        ...
        if 'KSH' in list_foods[i]:
            list_foods[i] = str(list_foods[i][:list_foods[i].index('KSH') - 1]) + ' ' * (
                        20 - (list_foods[i].index('KSH') - 1)) + str(list_foods[i][list_foods[i].index('KSH'):])
        i += 1

    i = 0
    while i <= (len(list_drinks) - 1):
        if 'KSH' in list_drinks[i]:
            list_drinks[i] = str(list_drinks[i][:list_drinks[i].index('KSH') - 1]) + ' ' * (
                        20 - (list_drinks[i].index('KSH') - 1)) + str(list_drinks[i][list_drinks[i].index('KSH'):])
        i += 1

    i = 0
    while i <= (len(list_services) - 1):
        if 'KSH' in list_services[i]:
            list_services[i] = str(list_services[i][:list_services[i].index('KSH') - 1]) + ' ' * (
                        20 - (list_services[i].index('KSH') - 1)) + str(list_services[i][list_services[i].index('KSH'):])
        i += 1
...
def def_file_sorter():  # Applying Sorting to the array to be sorted from A-Z ASC ((AND)) Extracting out prices after sorting and appending them to a prices array accordingly to a parrallel indexes
    ...
    while i < len(list_foods):
        list_item_price[i] = float(list_foods[i][int(list_foods[i].index(
            "KSH") + 3):])  # Extracting Out "RM" + [SPACE] from and cast out the string into an integer
        i += 1

    i = 0
    while i < len(list_drinks):
        list_item_price[40 + i] = float(list_drinks[i][int(
            list_drinks[i].index("KSH") + 3):])  # Applying extraction on 40 and above items which are the drinks
        i += 1

    i = 0
    while i < len(list_services):
        list_item_price[80 + i] = float(list_services[i][int(
            list_services[i].index("KSH") + 3):])  # Applying extraction on 80 and above items wich are Services
        i += 1
...
def def_other_services():
    while True:
        ...
        while i < len(list_services):
            print(" (" + str(81 + i) + ")" + " " + str(
                list_services[i]))  # Services starts from 81 + and now it is being enumarated into a list.
        ...
        try:
            ...
            if (int(input_1) > 80) and (int(input_1) < 100):
                print("\n" * 10)
                print("Successfully Ordered: " + str(list_services[int(
                    input_1) - 81]))  # Adding services to orders array (AND) encapsulate errors  with try/except
    ...
    while True:
        ...
        while i < len(list_item_order):  # Enumarating order array items and summing up its prices * quantities
            if (list_item_order[i] != 0):
                if (i >= 0) and (i < 40):
                    report_new += "\n" + " " * 17 + str(list_foods[i]) + "  x  " + str(list_item_order[
        ...
        if total_price > var_discount_3:  ### price > 5000
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" \
                ...
                var_discount_3_rate * 100) + "\n" \
                                             "" + " " * 17 + "DISCOUNT AMOUNTS:   KSH " + str(
                round(total_price * var_discount_3_rate, 2)) + "\n" + " " * 17 + "_" * 35 + "\n" \
                                                                                            "" + " " * 17 + "TOTAL PRICES:       KSH " + str(
            ...
            report_new += "\n" + " " * 17 + "-" * 35 + "\n" + " " * 17 + "TOTAL PRICES:       KSH " + str(
                round(total_price, 2)) + "\n" + " " * 17 + "*" * 35
            print(" " * 17 + "_" * 35 + "\n" + " " * 17 + "TOTAL PRICES:       KSH " + str(round(total_price, 2)))
